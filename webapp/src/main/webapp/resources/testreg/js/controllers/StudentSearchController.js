testreg.controller('StudentSearchController', ['$scope', '$state','$timeout','$window', 'EntityService', 'StudentService', 'StateService',
     function ($scope, $state,$timeout,$window, EntityService, StudentService, StateService ) {
	 ...
	 if(!$state.current.searchParams) {
		...
		$scope.changeDistrictId = function(districtId){
  		...
  		$scope.isAllOptionsChecked = function(reportType,mode,reportFormat) {
  			$scope.isDisabled = true;
  			if(angular.isDefined(reportType) && angular.isDefined(mode) && mode != null && angular.isDefined(reportFormat)){
